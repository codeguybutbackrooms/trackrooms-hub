...
local soulBtn = CreateButton(contentFrame, "Soul", "Soul Mode", UDim2.new(0,0,0,0))
...
soulBtn.MouseButton1Click:Connect(function()
    soulMode = not soulMode

    local char = localPlayer.Character
    local hum = char and char:FindFirstChild("Humanoid")

    if soulMode then
        soulBtn.Text = "Soul Mode: ON"
        ...
        camConnection = RS.RenderStepped:Connect(function()
            ...
            local finalSpeed = shiftHeld and 3 or 1

            camera.CFrame = camera.CFrame + move * finalSpeed
        end)

    else
        soulBtn.Text = "Soul Mode"
