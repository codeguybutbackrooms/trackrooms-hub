...
local showSoundsButton = CreateButton(contentFrame, "ShowSounds", "Show Sounds", UDim2.new(0, 190, 0, 280))
local soundIdInput = CreateTextInput(contentFrame, "SoundId (rbxassetid://...)", UDim2.new(0, 20, 0, 600))
local playSoundButton = CreateButton(contentFrame, "PlaySound", "Play Sound", UDim2.new(0, 190, 0, 600)
...
local function CreateSoundsUI()
    soundsFrame = Instance.new("Frame")
    soundsFrame.Size = UDim2.new(0, 420, 0, 300)
    soundsFrame.Position = UDim2.new(0.5, -210, 0.5, -150)
    soundsFrame.BackgroundColor3 = Color3.fromRGB(20,20,25)
    soundsFrame.BorderSizePixel = 0
    soundsFrame.Active = true
    soundsFrame.Draggable = true
    soundsFrame.Parent = modernScreenGui

    Instance.new("UICorner", soundsFrame).CornerRadius = UDim.new(0,14)

    local stroke = Instance.new("UIStroke", soundsFrame)
    stroke.Color = Color3.fromRGB(0,150,255)
    stroke.Thickness = 1.5
    stroke.Transparency = 0.3

    soundsScroll = Instance.new("ScrollingFrame")
    soundsScroll.Size = UDim2.new(1, -20, 1, -20)
    soundsScroll.Position = UDim2.new(0,10,0,10)
    soundsScroll.CanvasSize = UDim2.new(0,0,0,0)
    soundsScroll.ScrollBarThickness = 6
    soundsScroll.BackgroundTransparency = 1
    soundsScroll.Parent = soundsFrame

    soundLayout = Instance.new("UIListLayout")
    soundLayout.Padding = UDim.new(0,6)
    soundLayout.Parent = soundsScroll

    soundLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        soundsScroll.CanvasSize =
            UDim2.new(0,0,0,soundLayout.AbsoluteContentSize.Y + 10)
    end)
end

local function CreateSoundItem(sound)
    if soundItems[sound] then return end
    ...
    local function Update()
        item.Text = string.format(
            "%s | %s",
            sound.SoundId ~= "" and sound.SoundId or "No SoundId",
            sound.IsPlaying and "Playing" or "Stopped"
        )
    end
    ...
    item.MouseButton1Click:Connect(function()
        if sound.SoundId and sound.SoundId ~= "" then
            CopyToClipboard(sound.SoundId)

            item.Text = "Copied!"
            task.delay(0.4, Update)
        end
    end)

    local playConn =
        sound:GetPropertyChangedSignal("IsPlaying"):Connect(Update)

    sound.AncestryChanged:Connect(function(_, parent)
        if not parent then
            playConn:Disconnect()
            item:Destroy()
            soundItems[sound] = nil
        end
    end)

    soundItems[sound] = item
end


local function EnableShowSounds()
    CreateSoundsUI()
...
showSoundsButton.MouseButton1Click:Connect(function()
    showSoundsEnabled = not showSoundsEnabled
    showSoundsButton.Text =
        showSoundsEnabled and "Show Sounds: ON" or "Show Sounds"

    if showSoundsEnabled then
        EnableShowSounds()
    else
        DisableShowSounds()
    end
