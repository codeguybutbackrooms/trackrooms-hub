...
---------------------------------------------------------------------
function CreateCollapsible(parent, name, title, position)
    -- Main holder
    local holder = Instance.new("Frame")
    holder.Name = name
    holder.Parent = parent
    holder.Position = position
    holder.Size = UDim2.new(0, 340, 0, 40)
    ...
    header.Text = "▼  " .. title     -- default: CLOSED
    ...
    content.Size = UDim2.new(1, 0, 0, 0) -- collapsed
    ...
    local function toggle()
        ...
        if open then
            ...
            -- auto height theo nội dung
            local totalHeight = 0
            for _, v in ipairs(content:GetChildren()) do
                if v:IsA("Frame") or v:IsA("TextButton") then
                    totalHeight += v.AbsoluteSize.Y + 6
                end
            end
            ...
            TweenService:Create(holder, TweenInfo.new(0.25), {
                Size = UDim2.new(0, 340, 0, 40 + totalHeight)
            }):Play()
    ...
    return content -- để nhét CreateButton vào trong
end
---------------------------------------------------------------------
-- INPUTS + BUTTONS
---------------------------------------------------------------------
...
-- playersSection is the collapsible created earlier
-- contentFrame inside the collapsible
local content = playersSection.ContentFrame -- assuming CreateCollapsible returns the contentFrame
...
local function RefreshPlayersList()
    ...
    -- Create a button for each player (except local player)
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= Players.LocalPlayer then
            ...
            button.Text = "Teleport to " .. p.DisplayName

            -- Teleport local player to the clicked player's HumanoidRootPart
            button.MouseButton1Click:Connect(function()
                ...
                if localPlayer.Character and p.Character then
                    ...
                    if hrp1 and hrp2 then
                        hrp1.CFrame = hrp2.CFrame + Vector3.new(0,2,0)
                    end
                end
            end)
        end
...
-- Refresh the player list when the script runs
RefreshPlayersList()

-- Refresh the player list when a new player joins
Players.PlayerAdded:Connect(function()
    RefreshPlayersList()
end)

-- Refresh the player list when a player leaves
